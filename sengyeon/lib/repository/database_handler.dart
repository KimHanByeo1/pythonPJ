...
//삭제
  ...
  // title과 category column으로 검색하는 query
  Future<List<Calendar>> textSearchList(
      String? searchText, String? searchBtn) async {
    final Database db = await initializeDB(); // 생성한 Table 선언

    // 생성한 Table 내의 데이터를 Select로 불러오는 Query
    final List<Map<String, Object?>> queryResult = await db.rawQuery("""
          SELECT * FROM calendar 
          WHERE title LIKE ? AND category 
          LIKE ? ORDER BY writeday DESC
        """,
        // like문에 들어갈 값을 전달받은 매개변수로 값을 대체함
